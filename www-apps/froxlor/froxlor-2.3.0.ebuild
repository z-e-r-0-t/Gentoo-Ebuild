...
LICENSE="GPL-2"
SLOT="0"
IUSE="+apache2 awstats bind fcgid +fpm +goaccess mailquota nginx pdns +proftpd pureftpd quota +rspamd +ssl webalizer"
KEYWORDS="~amd64 ~x86"
...
src_prepare() {
	...
	if is_systemd; then
		...
		einfo "Patching LE renew hook"
		patch_defaults "system" "le_renew_hook" "/usr/sbin/froxlorRestartHookLE.sh"
	fi
...
src_install() {
	...
	if ! is_systemd; then
		dosbin "${FILESDIR}/froxlorRestartHookLE.sh"
	fi
