...
LICENSE="GPL-2"
SLOT="0"
IUSE="awstats bind +dovecot fcgid +fpm ftpquota +goaccess lighttpd +log mailquota nginx pdns pureftpd quota +ssl"
...
REQUIRED_USE="lighttpd? ( !nginx ) fcgid? ( !fpm ) pdns? ( !bind ) awstats? ( !goaccess )"
...
FROXLOR_DOCROOT="${FROXLOR_DOCROOT:-/var/www/html}"
...
src_install() {
	...
	if use l10n_de ; then
		...
		MYLANG="cz"
	elif use l10n_es ; then
		MYLANG="es"
	fi

	if [[ ${MYLANG} != '' ]] ; then
		einfo "Setting standardlanguage to '${MYLANG}'"
		sed -e "s|'standardlanguage', 'en'|'standardlanguage', '${MYLANG}'|g" -i "${S}/install/froxlor.sql.php" || die "Unable to change default language"
	fi
	...
	if use lighttpd; then
		...
		sed -e "s|service apache2 reload|/etc/init.d/lighttpd restart|g" -i "${S}/install/froxlor.sql.php" || die "Unable to change webserver restart-command"
		...
		sed -e "s|service apache2 reload|/etc/init.d/nginx restart|g" -i "${S}/install/froxlor.sql.php" || die "Unable to change webserver restart-command"
...
#		einfo "Setting configdir for php-fpm"
#		sed -e "s|(1, 'System default', 'service php7.4-fpm restart', '/etc/php/7.4/fpm/pool.d/')|(1, 'System default', 'service php7.4-fpm restart', '/etc/php/7.4/fpm.d/')|g" -i "${S}/install/froxlor.sql.php" || die "Unable to set configdir for 'fpm'"
	...
	if use bind ; then
		...
		sed -e 's|service bind9 reload|/etc/init.d/named reload|g' -i "${S}/install/froxlor.sql.php" || die "Unable to change reload path for Bind"
	fi

	if use pdns ; then
		einfo "Switching from 'bind' to 'powerdns'"
		sed -e 's|'bind_enable', '0'|'bind_enable', '1'|g' -i "${S}/install/froxlor.sql.php" || die "Unable to change binds enabled flag"
		sed -e 's|service bind9 reload|/etc/init.d/pdns restart|g' -i "${S}/install/froxlor.sql.php" || die "Unable to change reload path for pdns"
	...
	if use awstats ; then
		einfo "Switching from 'Goaccess' to 'AWStats'"
		sed -e "s|'traffictool', 'goaccess'|'traffictool', 'awstats'|g" -i "${S}/install/froxlor.sql.php" || die "Unable to enable AWStats"
	fi

	if ! use awstats && ! use goaccess ; then
		einfo "Switching from 'Goaccess' to 'Webalizer'"
		sed -e "s|'traffictool', 'goaccess'|'traffictool', 'webalizer'|g" -i "${S}/install/froxlor.sql.php" || die "Unable to enable webalizer"
	fi
